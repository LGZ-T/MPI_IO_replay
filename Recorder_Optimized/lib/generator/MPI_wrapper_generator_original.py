...
# generate function body
	...
	call_para = '('
	extra_data = ''
	print_format_1 = '"tm1=%.5f tm2=%.5f ' + para_list[0].lstrip('P') + '('
	...
	for para in paras:
		...
		if extra is not None:
			extra_data += extra
			extra = None
			print_format_1 += true_para + '=' + para_format.get(type_of_para, 'Error') + ', '
			print_format_2 += ', ' + true_para + '_name'
		elif pointer == True:
			# for void *buf, int *comm, int *size, etc.
			if type_of_para == 'void' or type_of_para == 'int':
				print_format_1 += true_para.lstrip('*') + '=' + true_para + ', '
			else:
				print_format_1 += true_para.lstrip('*') + '=' + para_format.get(type_of_para, 'Error') + ', '
				if type_of_para == 'MPI_File': # only use address
					#print_format_2 += ', ' + true_para
					print_format_2 += ', ' + true_para.lstrip('*')
				else:
					print_format_2 += ', ' + true_para.lstrip('*')
	...
	print_format_1 = print_format_1[:-2]
	print_format_1 += ')\\n",'
...
#	body += '\tif (__recorderfh != NULL)\n'
	body += '\tbytes = sprintf(rec_buffer + written_bytes, ' + print_format_1 + print_format_2 + ');\n'
...
if __name__ == '__main__':
	...
	input_header = open('input_header')
